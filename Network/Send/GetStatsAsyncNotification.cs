...
// Type: BlazeServer.Network.Send.GetStatsAsyncNotification
...
internal class GetStatsAsyncNotification : SendPacket
{
  private List<long> eidlist;
  private List<string> statvalues;
  private long VidValue;

  public GetStatsAsyncNotification(List<long> eidlist, List<string> statvalues, long VidValue)
  {
  ...
  {
    this.WriteStr("GRNM", "player_mpdefault2");
    ...
    this.WriteStruct("STS\0", new List<Blaze.Tdf>()
    {
      (Blaze.Tdf) this.SetList("STAT", (byte) 3, 1, (object) new List<Blaze.TdfStruct>()
      {
        this.SetStruct("0", new List<Blaze.Tdf>()
        {
          ...
          (Blaze.Tdf) this.SetList("STAT", (byte) 1, 1432, (object) this.statvalues)
        })
      })
    });
